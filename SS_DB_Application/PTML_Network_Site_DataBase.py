...
# Initial PMO Status options
pmo_status_options = ["CL", "NCL", "NA", "NCL-relocation", "Planned"]

# Ask user how many new options they want to add
num_new_options = st.number_input("How many PMO Status values do you want to add?", min_value=0, step=1)

# Let user add new PMO Status values
for i in range(num_new_options):
    new_status = st.text_input(f"Enter new PMO Status {i+1}:", key=f"new_status_{i}")
    if new_status:
        pmo_status_options.append(new_status)
...
for status in pmo_status_options:
    default_checked = status in ["CL", "NCL"]
    if st.checkbox(status, value=default_checked, key=status):
        selected_pmo_status.append(status)
